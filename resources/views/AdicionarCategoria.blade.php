...
<form method="POST" action="{{ route('register') }}" class="justify-self-center flex my-33">
    ...
    <div class="h-173 w-130 bg-linear-to-t/shorter from-red-500 to-rose-950 rounded-2xl shadow-xl shadow-red-700/60">
        ...
        <h2 class="justify-self-center text-4xl font-extrabold text-slate-50 mt-4" style="font-family: Nunito">Adiconar Categoria</h2>

        <input type="text" id="name" name="name" placeholder="Nome da categoria *" style="font-family: Nunito" class="justify-self-center flex rounded-xl w-80 h-12 pl-2 mt-8 text-slate-50 bg-indigo-950" required autofocus autocomplete="name">
        <x-input-error :messages="$errors->get('name')" class="mt-2 ml-26 w-80" />

        <input type="number" id="email" name="text placeholder="Quantidade de veículos *" style="font-family:Nunito" class="justify-self-center flex rounded-xl w-80 h-12 pl-2 mt-8 text-slate-50 bg-indigo-950" required autocomplete="username">
